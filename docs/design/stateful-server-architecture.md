...
History Shards and Matching Shards are **independently sharded**. A history shard is keyed by a hash of `job.id`; a matching shard is keyed by a hash of `(job.groupId, job.type)`. The same node may own some history shards, some matching shards, neither, or both — assignment is independent.

> **Terminology note.** This document uses the existing LogPlay domain model consistently: jobs have an `id`, a `groupId`, a `name`, a `type`, and pass through `JobStatus` (`PENDING` → `ACQUIRED` → `FINISHED | FAILED | ABORTED`). The job's `groupId` is a multi-tenant group identifier; the job's `type` — an opaque string label set by the client at creation time (e.g., `"send_email"`, `"resize_image"`) — is what workers declare when they register and what Matching uses for dispatch routing. Workers poll by `(groupId, type)`. Throughout this document, "job type" and `job.type` always refer to the type field.

### Why this split?

History shards exist because **per-job state needs single-writer locality**. Matching shards exist because **workers want to subscribe to (group, type) pairs, not job IDs**. If history and matching were the same sharding axis, a worker for `send_email` would have to long-poll every history shard. Splitting them lets one worker connect to one matching shard for its (group, type), regardless of which history shards the actual jobs live in.
...
3. Worker is long-polling matching shard owner for its (groupId, job type). Matching delivers the job descriptor inline. Worker starts execution.
...
3. HISTORY ENGINE's TransferQueueProcessor   [background loop on same pod]
     ...
     │   hash(groupId, "send_email") mod N_M
     ▼
4. MATCHING ENGINE   [pod that owns the matching shard for (groupId, "send_email")]
     │
     │ Purely in-memory:
     │   if a worker is parked long-polling for (groupId, "send_email"):
...
6. HISTORY ENGINE
     ...
     │   IF range_id = <current>
     │
     │ Returns (input_data, lease_deadline, range_token) to the worker.
     │ This is the ONLY place input_data flows out of History — Matching
     │ never sees it, which is what bounds Matching's in-memory footprint
     │ (see §5.3 *In-memory footprint and retention*).
     ▼
7. Worker executes the job (with input_data received in step 6).
   Periodically:
...
```
matching_shard_id = hash(job.groupId, job.type) mod N_M
```

Note: this means **all jobs of the same (group, type) pair land on the same matching shard owner**. This provides tenant isolation at the matching layer — group A's `send_email` and group B's `send_email` can land on different matching shards. If one (group, type) pair has overwhelming traffic, that matching shard becomes hot. Mitigation: introduce sub-sharding for job types with prefix conventions like `send_email_0`, `send_email_1`, ..., handed out by the SDK — see §11 Q5 for the full discussion. Out of scope for v1.
...
```
job_id = deterministic_hash(groupId, idempotency_key)   // UUIDv5(NAMESPACE, encodeName(groupId, idempotencyKey))
history_shard_id = hash(job_id) mod N_H
```

Because `job_id` is a pure function of `(groupId, idempotency_key)`, two clients submitting the same (group, key) pair always produce the same `job_id` and route to the same history shard. The shard owner sees the second `INSERT` arrive for an existing `job_id` and returns the existing job (idempotent). No global uniqueness index needed. Two different groups using the same idempotency key produce different job IDs — idempotency is scoped per group, matching the current design.
...
backoff_map:  Map<(NodeId, GroupId, JobType), BackoffState>  -- per-destination, per-(group, type) pause state
```
...
loop every LOOP_INTERVAL:
  ...
  for row in rows:
    ...
    matching_shard = hash(row.group_id, row.job_type) mod N_M
    ...
    # 2. Check local backoff state for this destination + (group, type)
    if backoff_map[(target_node, row.group_id, row.job_type)].paused_until > now:
    ...
    result = channel.offerJob(OfferJobRequest {
      ...
      visibility_time  = row.visibility_time,   # age key for matching's heap
      attempt          = row.attempt,
      # NOTE: input_data is deliberately NOT carried here. Matching holds
      # metadata only; the worker fetches input_data from the history shard
      # on AcceptJob (see §5.3 in-memory footprint, and §8.2 AcceptJob flow).
    })
    ...
    match result:
      case Accepted:
        ...
        backoff_map[(target_node, row.group_id, row.job_type)].reset()
      ...
      case QueueFull { current_depth, max_depth }:
        ...
        backoff_map[(target_node, row.group_id, row.job_type)].record_rejection()
      ...
      case RpcError:
        ...
        backoff_map[(target_node, row.group_id, row.job_type)].record_failure(short_duration = 2s)
...
| `ready_queues: Map<(GroupId, JobType), ReadyQueue>` | One bounded min-heap of ready job offers per (group, type) pair, ordered by `visibility_time` (oldest first). |
| `pollers: Map<(GroupId, JobType), Queue<PollerSession>>` | Workers currently long-polling this matching shard, by (group, type). |

A `ReadyQueue` is a bounded in-memory min-heap of `JobOffer { job_id, history_shard_id, group_id, job_type, visibility_time, attempt }`, keyed on `visibility_time` so the oldest-eligible offer is always at the head. It carries a dedup index `Set<JobId>` to prevent the same job from appearing twice, and a max size (`MAX_QUEUE_DEPTH`, default 10,000 per (group, type) pair). Insert and pop are O(log n); at a 10K cap, that is ~13 comparisons — negligible compared to the gRPC cost of a single offer or poll.

**`input_data` is deliberately absent from `JobOffer`.** Matching holds metadata only. The worker fetches `input_data` from the history shard when it calls `AcceptJob` (§8.2), so the payload never lives in matching's RAM. This is the key invariant that keeps matching's aggregate memory predictable regardless of job input sizes — see §5.3's *In-memory footprint and retention* subsection below for the full sizing story.
...
handle_offer_job(req):
  key = (req.group_id, req.job_type)
  queue = ready_queues.get_or_create(key, max_size = MAX_QUEUE_DEPTH)

  # Fast path: a worker is already parked for this (group, type)
  if pollers[key].has_parked():
    poller = pollers[key].pop()
  ...
  # Normal path: insert into the heap, keyed by req.visibility_time
  queue.insert(req)   # O(log n); heap head = oldest visibility_time
  return OfferJobAck { accepted: true }
```
...
```
handle_poll_job(req):
  key = (req.groupId, req.job_type)
  queue = ready_queues.get_or_create(key)

  # Fast path: work is already queued
  if queue.has_offers():
    offer = queue.pop_oldest()    # heap min by visibility_time
    return PollJobResponse { offer }

  # No work — park the poller
  poller_session = create_poller_session(req.worker_id, key)
  pollers[key].append(poller_session)
  ...
  if poller_session.has_offer():
    ...
    # Worker SDK retries with a new long-poll — possibly on a different
    # job_type per its rotation strategy (see §7.2).
```

Matching has **no opinion** on how the worker picked `req.job_type`. Every `PollJob` is one worker parked on exactly one (group, type) pair on exactly one matching shard, regardless of whether the worker handles a single type or rotates through many. Rotation is entirely the client SDK's concern (§7.2.2).
...
#### Cross-shard fairness ordering

Today's LogPlay server implements a soft "oldest-first" priority by ordering `acquirePendingJobs` on `updated_at ASC`. This gives a global FIFO-ish fairness within a `(groupId, type)` and prevents starvation under contention. The stateful-server model preserves the same property in two layers:

- **Within a history shard**: `transfer_tasks` is read by `ORDER BY task_seq`, which is monotonic within the shard. Oldest-eligible transfer task goes out first.
- **Across history shards**: the matching shard's `ReadyQueue` is a min-heap keyed on `visibility_time`, so when N history shards simultaneously feed offers for the same job type, the heap merge dispatches globally-oldest-first.

The **age key is `visibility_time`, not `job.created_at`**. Why:

- For a fresh job, `visibility_time = created_at` — identical semantics.
- For a delayed or retry-backed-off job, `visibility_time = when it became eligible to run` — which is the moment it should start competing for dispatch, not when it was originally created. This matches today's behavior where `updated_at` is bumped on release/retry.
- A retried job therefore takes its turn at the back of the heap, exactly like today's release-bumps-updated-at pattern. A poison-pill job can't starve fresh work by keeping its original creation timestamp pinned at the head.

**Clock skew tolerance.** `visibility_time` is assigned by whichever history shard owner wrote the `transfer_tasks` row. Matching compares `visibility_time` values across shards owned by potentially different pods. Second-scale NTP skew reorders offers by a few positions in the heap but does not break correctness — it only slightly degrades the precision of "oldest first." Sub-second skew is below the noise floor of the dispatch path itself.

**Hot-shard edge case.** If a burst creates many jobs in one history shard and its `TransferQueueProcessor` backs off against a saturated matching shard, rows pile up in that shard's `transfer_tasks` while other (cooler) shards continue to offer freely. Once backoff expires, the piled-up rows — which have old `visibility_time` values — flood into matching's heap and naturally rise to the head of the queue, getting dispatched ahead of newer work from the cooler shards. **The backlog drains in age order automatically.** No cross-shard coordination needed.

#### In-memory footprint and retention

Because matching pre-fetches offers into RAM, it is worth spelling out exactly **how long an offer lives in memory** and **how much RAM it costs**. These properties are what make the bounded-in-memory-queue design safe in practice.

**Retention policy.** An offer enters a `ReadyQueue` when history's `TransferQueueProcessor` sends an `OfferJob` RPC to matching. Once in the heap, it lives there until one of these events happens:

1. A worker pops it via `PollJob` (the happy path — typically sub-second under healthy load).
2. The matching shard is **reassigned** — membership changes, the current owner drops the shard, a new owner acquires it. The new owner starts with empty heaps.
3. The owning pod **restarts or crashes** — in-memory state goes with the process.

There is **no TTL, no idle eviction, no expiry clock, no periodic sweep**. An offer for a type that has no worker can sit in the heap indefinitely — "until ownership moves" could be days under stable cluster membership.

**Why no eviction?** Matching has no durable state. Dropping an offer cannot lose data — history's `transfer_tasks` row is still there and it is the source of truth. But dropping does not help sustained RAM either, because history's `TransferQueueProcessor` re-offers the row on its next `REOFFER_INTERVAL` cycle (default 5 minutes). Dropping therefore buys at most 5 minutes of RAM relief in exchange for a 5-minute dispatch delay for that job. That is a bad trade. The correct policy is "keep everything you are handed and rely on `MAX_QUEUE_DEPTH` plus backpressure for the hard bound."

**The hard bound is `MAX_QUEUE_DEPTH`.** Each per-type `ReadyQueue` refuses new offers beyond this cap and NACKs with `QueueFull`. History backs off (§5.2.1) and the offers stay on disk in `transfer_tasks` instead of in matching's RAM. **In-memory pressure converts to disk-space lag** — memory is finite and expensive, disk is cheap and practically unbounded. No pod OOMs.

**Crash recovery.** When a matching shard moves (graceful transfer, pod crash, or ownership shift), the new owner starts with empty heaps. History's per-shard `TransferQueueProcessor` re-sends every unprocessed row via `OfferJob`, typically within one loop interval (500 ms idle + the time to drain the backlog through the RPC channel). No data is lost and no offer is missed; there is a brief dispatch pause during reconstruction that is bounded by the number of pending rows divided by the RPC throughput of a single pod-to-pod channel.

**Per-offer size breakdown.** A single `JobOffer` in the heap carries these fields (approximate JVM sizes, after boxing and object header overhead):

| Field | Approximate size |
|---|---|
| `job_id` (UUID or string) | 16–40 B |
| `history_shard_id` (boxed `Int`) | ~16 B |
| `job_type` (String, typical 32–64 chars) | 40–80 B |
| `visibility_time` (`Instant`) | ~24 B |
| `attempt` (boxed `Int`) | ~16 B |
| Heap-node object overhead + dedup `Set<JobId>` entry | 40–60 B |
| **Per-offer metadata subtotal** | **~150–250 B** |

Notably absent: `input_data`. Matching does not hold payloads — the worker fetches them from history on `AcceptJob` (§8.2). This is the load-bearing choice for the whole footprint story; without it, every number below would be multiplied by `sizeof(input_data)`.

**Aggregate worst case per matching shard.** A matching shard owns some number of job types (the number depends on `N_M` and the cluster's total distinct-type count). Assume 10 types per shard as a representative default. With `MAX_QUEUE_DEPTH = 10,000` per type, a fully saturated shard holds **100,000 offers**:

| Component | Aggregate worst-case RAM |
|---|---|
| Metadata only (10 types × 10K × ~200 B) | **~20 MB per matching shard** |
| Dedup indices and heap overhead | already included above |
| `input_data` (lazy fetch) | **0 bytes** — never stored in matching |

**20 MB per matching shard, regardless of input payload size.** A pod owning ~20 matching shards still fits comfortably in a small container.

For comparison, if `OfferJob` had carried `input_data` (the naive design), the numbers would have been:

| Avg `input_data` size | Worst-case RAM per matching shard |
|---|---|
| 1 KB | ~120 MB |
| 10 KB | ~1 GB |
| 256 KB | **~26 GB** |
...
— which is why the payload was factored out into lazy fetch. `MAX_QUEUE_DEPTH` alone is not a sufficient bound once payloads are in the picture; the bound has to come from keeping payloads off matching entirely.

**Sizing guidance.** For operators who want to tune `MAX_QUEUE_DEPTH` up or down, the formula is:

```
expected_shard_ram ≈ num_types_on_shard × MAX_QUEUE_DEPTH × 200 B
```

The 200 B factor is the "heavy" end of the metadata size range and is safe to use for capacity planning. At the default 10K cap and 10 types per shard, that is ~20 MB per shard — well under typical pod limits. Tuning `MAX_QUEUE_DEPTH` up to 100K/type would raise the worst case to ~200 MB per shard, still fine; tuning it down is only interesting if you want harder backpressure sooner (offers go to disk earlier under load).
...
Worker → Frontend: RegisterWorker {
  worker_id, groupId, job_types: ["send_email", "process_image"],
...
A worker registers for a specific `groupId` and a set of `job_types` within that group. The `groupId` scopes all the worker's subsequent `PollJob` requests. The `job_types` list is what this worker rotates its poll across (see §7.2) — it is **not** a fan-out subscription that opens N parallel streams to N matching shards. One worker instance = one poll stream at a time. A process that needs more parallelism instantiates multiple workers, each with its own `worker_id` and (optionally) a different `groupId` or `job_types` list.

### 7.2 Polling for Work

A worker holds **one `PollJob` stream at a time** and rotates its registered `job_types` across successive polls. It is not a fan-out subscription. Each `PollJob` targets exactly one job type and parks on the single matching shard that owns that type. When the poll returns (an offer or an empty timeout), the worker picks the next type per its rotation strategy and starts a new `PollJob`.

```
Worker → Frontend: PollJob {
  worker_id,
  groupId,                  # group scope — matches the worker's registered groupId
  job_type,                 # one type per call, chosen by the worker's SDK
  max_concurrent_slots,     # worker-wide in-flight budget; see below
  max_park_duration_ms      # client-requested long-poll length; capped server-side
}

Frontend computes matching_shard_id = hash(groupId, job_type) mod N_M
...
[time passes, bounded by max_park_duration_ms, default ~5–10s for multi-type workers]

A JobOffer arrives for this job type → Matching pops the parked poller,
OR the park duration elapses → Matching returns an empty response.

Matching → Worker: PollJob.Response {
  offer?: {
    job_id, history_shard_id, job_type,
...
The worker now has a `JobOffer` (or nothing). **The job is not yet ACQUIRED in storage** — that transition happens when the worker explicitly accepts the offer (see §8.2). The offer is just "here's work you can claim if you want it."

> **`input_data` is not in `PollJob.Response`.** Matching holds only metadata in its `ReadyQueue` (§5.3, *In-memory footprint and retention*). The worker receives `input_data` inside the `AcceptJob.Response` from the history shard when it commits to running the job. See §8.2 for the exact sequence.
...
#### 7.2.1 Protocol vs SDK strategy

The wire protocol deliberately knows nothing about how a worker picks its next `job_type`. It only enforces three things:

- **One `job_type` per `PollJob`.** No multi-type subscription, no fan-out.
- **Server-capped park duration.** The server enforces a maximum long-poll length (default 30s); a worker may request less via `max_park_duration_ms`. The cap is the server's guarantee that a stuck or forgotten park cannot monopolise a worker indefinitely.
- **`max_concurrent_slots` is worker-wide, not per-type.** A worker that advertises 10 slots will run at most 10 concurrent jobs across all its polls in aggregate — not 10 per type. The worker SDK is responsible for pausing new polls once its in-flight count hits the slot cap.

Everything else — which type to poll next, how long to park, how to handle empty responses — is SDK-side policy. Dumb SDKs can round-robin the `job_types` list and still be correct; smart SDKs can do better.

#### 7.2.2 Recommended SDK rotation strategy

Blind round-robin is wasteful whenever the load across a worker's types is skewed — which it almost always is. The worker burns its poll budget on cold types and is slow to pick up work on hot ones. The recommended default for the official SDK is a lightweight **multi-armed bandit**:

- **Per-type hit-rate EMA.** After each `PollJob` response, update an exponential moving average for that type: `hit = 1` if an offer was returned, `0` if empty. An α of ~0.2 gives reasonable smoothing.
- **Weighted selection.** The next poll's `job_type` is drawn in proportion to the EMAs. Hot types get most of the polls; cold types get few.
- **ε-exploration.** With probability ~5–10%, ignore the weights and pick uniformly at random. This is what prevents permanent starvation of a type that suddenly wakes up after a long quiet period.
- **Short effective park duration for multi-type workers.** Rather than parking 60s on one type, request `max_park_duration_ms ≈ 5–10s`. This bounds how long the worker stays committed to a type that might have gone cold since the last hit. For workers with exactly one type, a longer park (up to the server cap) is fine — the trade-off only matters when there are multiple types competing for the single stream.

Steady-state behavior under this strategy:

- **Single-type worker** (`job_types: ["send_email"]`): weights trivially converge to "always poll send_email." Matches the behavior you'd get from a dedicated single-type stream. No overhead.
- **Multi-type worker with skewed load** (e.g. 90% of jobs are type A, 10% are type B): weights concentrate polls on A; ε-exploration occasionally visits B; the worker picks up both types with effective latency close to what a dedicated stream per type would give.
- **Multi-type worker with uniform load**: weights remain roughly equal; the strategy degenerates to round-robin, which is correct for this case.
- **Multi-type worker where one type suddenly wakes up after being cold**: ε-exploration discovers the wake-up within `~1/ε` polls. With ε = 0.1 and 5s polls, that is ~50s worst-case detection.

#### 7.2.3 Escape hatch: latency-sensitive cold types

The one workload the rotation model cannot serve well is a **cold type with a strict sub-second latency SLO** — a job type that gets traffic rarely but must be picked up immediately when it does. No bandit strategy can deliver this, because any rotation across types inherently risks missing the wake-up window.

The clean answer is to use the existing primitive: **instantiate a dedicated worker for that type.**

```
# worker process A — the bulk worker, rotates over the common types
worker_a = Worker(worker_id="pod-1-bulk", groupId="order-service", job_types=["send_email", "resize_image", "index_document"])

# worker process B — dedicated to the latency-sensitive type, permanently parked on it
worker_b = Worker(worker_id="pod-1-critical", groupId="order-service", job_types=["process_refund"])
```

Worker B has only one type, so its bandit weights converge to "always poll `process_refund`" and it parks permanently on the single matching shard that owns that type. It gets the original "one stream, always listening" behavior for free. The two workers share the same pod / deployment but are independent at the protocol level — two rows in the `workers` table, two independent heartbeats, two independent leases.

This composition keeps the protocol clean (no "priority types" concept, no "pinned subscriptions") while still letting operators tune their deployments for mixed latency profiles. The cost of a dedicated worker is one extra registration and one extra long-poll connection — trivial.
...
HSO mailbox processes:
  ...
  Read jobs.input_data (already in memory from the row read above, or
    fetched from storage if not cached).
  Reply success with input_data + lease deadline.
...
Worker begins executing with input_data.
```

**Why `input_data` returns here, not on `PollJob`.** `AcceptJob` is the first moment the worker has committed to actually running this job — it is already inside the history shard's mailbox, the fenced state transition to `ACQUIRED` has just been written, and the jobs row is in memory on the history shard owner. Returning `input_data` in this response costs nothing beyond a slightly larger response body; there is no extra RPC round-trip. The alternative — carrying `input_data` all the way through `OfferJob` → `PollJob.Response` — would force matching to buffer payloads in RAM for every pending offer, turning a ~20 MB/shard footprint into tens of GB under realistic workloads (see §5.3, *In-memory footprint and retention*). Lazy fetch is the load-bearing choice that keeps matching's memory bounded regardless of payload size.
...
### Q8b: How does a worker subscribe to multiple job types?

A worker declares `groupId` and `job_types: [...]` at registration, but the wire protocol is one `PollJob` per `(groupId, job_type)` at a time (§7.2). The worker SDK rotates its single stream across the declared types within its group. The rotation strategy lives **entirely in the SDK**, not the server — the server's only per-type enforcement is that each `PollJob` parks on exactly one matching shard until it returns or the server-capped park duration elapses.

**Why not fan out to N parallel streams?** Each job type hashes to a different matching shard, so a fan-out model means `N_types` parallel long-polls per worker, multiplied across the whole cluster. For workers that handle 5–10 types, this inflates connection count by an order of magnitude without proportionate benefit, because real workloads almost always have skewed per-type traffic — most of the parked streams sit idle most of the time. A single rotating stream with a smart selection policy captures most of the benefit at 1/N the connection cost.

**Why not blind round-robin?** Because it *is* wasteful under skewed load. A worker with 10 types and one hot type would spend 90% of its polls on cold types while the hot type's work waits. The recommended SDK default (§7.2.2) is a weighted multi-armed bandit: per-type EMA on hit-rate, ε-exploration to discover re-awakened types, and a short effective park duration (5–10s) to keep rotation responsive.

**Trade-off profile:**

| Worker profile | Behavior under rotation + bandit |
|---|---|
| Single type | Weights converge trivially; behaves exactly like a dedicated stream. No overhead. |
| Skewed multi-type (common) | Concentrates polls on hot types; cold types served within `~1/ε × park_duration`. |
| Uniform multi-type (rare) | Degenerates to round-robin; correct for this case. |
| Multi-type with a latency-sensitive cold type | Bandit cannot serve sub-second SLOs on cold types. **Use the escape hatch**: instantiate a dedicated worker for that type (§7.2.3). |

**Escape hatch recap.** A worker with `job_types: [X]` naturally parks permanently on X because its bandit has nothing else to pick. Deploying two workers side-by-side in the same process — one bulk rotator for common types, one single-type worker for the latency-sensitive type — gives mixed-latency workloads the right tool without polluting the protocol with priority tiers or pinned subscriptions.

**Open knob**: the server-enforced max park duration. Default 30s feels right — long enough that single-type workers stay efficient (few RPCs per minute), short enough that a stuck park cannot hold the worker hostage. Tuneable per-cluster.
...
| **Long-poll** | The dispatch mechanism: a worker opens a `PollJob` stream for one `job_type` at a time and is parked on the owning matching shard until an offer arrives or the server-capped park duration elapses. A worker with multiple declared types rotates a single stream across them; it does not fan out. |
| **ReadyQueue** | Per-job-type, per-matching-shard, in-memory bounded min-heap of pending `JobOffer`s, ordered by `visibility_time` (oldest first). Holds no durable state; re-fed by history shards on takeover. |
| **Rotation / bandit polling** | The SDK-side strategy by which a worker picks which `job_type` to target on its next `PollJob`. Recommended default is a weighted multi-armed bandit with EMA hit-rate and ε-exploration (§7.2.2). |
...
## Appendix C — Functional Comparison to Current Design

This section inventories every functional difference between the **current** LogPlay server (see `docs/design/DESIGN.md`) and the stateful-server architecture described above. It exists because a migration is not just a scaling story — some things the current design does trivially become hard or impossible in the new model, and those need to be surfaced *before* anyone commits to the rewrite.

The comparison is organized into three buckets:

- **Hard losses** — capabilities that are gone or meaningfully harder, where users of the current design will notice the difference.
- **Soft losses / trade-offs** — things that change shape in ways worth naming, but that most users won't hit.
- **Moved but equivalent** — things that get re-implemented against new primitives but preserve their functional contract.

A final subsection lists the open decisions that this architecture forces and that a v1 commitment must answer.

### C.1 Hard Losses

#### C.1.1 `groupId`-based multi-tenancy is preserved

The current design lets multiple client services share a single LogPlay deployment by scoping everything to a `groupId`: workers poll by `(groupId, type)` and are guaranteed to only see that tenant's jobs of a specific type; idempotency keys are unique *per group*; jobs are dispatched within a (group, type) scope.

The new design preserves `groupId` as a first-class concept. Workers register with a `groupId` and poll by `(groupId, type)`. Matching shards are keyed by `hash(groupId, type)`, providing tenant isolation at the matching layer — group A's `send_email` and group B's `send_email` can land on different matching shards. Idempotency keys remain scoped per group (since `job_id = UUIDv5(groupId, idempotencyKey)`). No SDK-level type namespacing is required.

The three concerns from the original analysis are resolved:

- **Tenant isolation is enforced, not convention.** A worker registered for the wrong group cannot receive another tenant's jobs — the groupId is part of the matching shard key.
- **Per-tenant fairness is preserved.** Each (group, type) has its own `ReadyQueue` heap — tenants do not compete in shared heaps.
- **Per-tenant quotas and throttling retain their hook.** The `groupId` is available on every request for filtering and rate-limiting.

#### C.1.2 Idempotency-key scope remains per-group

Because `job_id = UUIDv5(groupId, idempotency_key)`, the same key used by two different groups produces different job IDs and routes to different history shards. No collision. This matches the current design exactly — idempotency keys are unique per group, not globally.

#### C.1.3 Ad-hoc queries on the jobs table are gone

This is the biggest functional regression and the one most likely to affect real users. The current design runs on one logical `jobs` table and you can query it freely:

```sql
-- Debugging: show me recent failures for a tenant
SELECT * FROM jobs WHERE status='FAILED' AND groupId=X LIMIT 100;

-- Searching: find a job by name
SELECT * FROM jobs WHERE name LIKE 'process-order-%';

-- Monitoring: how much work is pending
SELECT COUNT(*) FROM jobs WHERE status='PENDING';

-- Activity feed: what changed in the last hour
SELECT * FROM jobs WHERE updated_at > now() - interval '1 hour';
```

In the new design, **none of these are directly possible**. Jobs are sharded across `N_H` history shards keyed by `hash(job_id)`. Any query that doesn't know the `job_id` up front requires a fan-out read to every shard — unbounded latency, unbounded storage IO, and the shard owners don't even expose a "list my jobs by status" RPC today.

§11 Q6 acknowledges this and punts to "use CQRS — populate a downstream read model from `job_events` into Elasticsearch / ClickHouse / a Postgres replica." This is a real answer for admin UIs, but it is **not a drop-in replacement**:

- It adds a second storage system to the deployment.
- It is eventually consistent — the read model lags the canonical state.
- Populating it requires a background job_events consumer on every history shard.
- Every query that currently lives in SQL must be rewritten against the read model's query language.

**This is the single item on this list that users most need to understand before migrating.** If a team currently relies on ad-hoc SQL against the jobs table for debugging, monitoring, or support workflows, they need an alternative in place *before* the new server ships, not after.

#### C.1.4 Global cluster-wide counters and stats

Same root cause as C.1.3. "How many jobs are `PENDING` cluster-wide right now?" is one `SELECT COUNT(*)` today; it's a fan-out across every history shard tomorrow, or a read-model query against a separate system.

Cluster-level metrics (Prometheus counters for `pending`, `acquired`, `failed_24h`, etc.) can still be produced, but they must be emitted from each history shard owner and aggregated externally — they cannot be computed by a single query.

#### C.1.5 Simple database-level debugging workflow

Today: when something goes wrong in production, an operator opens `psql` / `h2-console`, runs a query against the jobs table, and sees what's happening. The barrier to "look at what the system is doing" is one connection string.

Tomorrow: state is spread across `shard_meta`, `jobs`, `transfer_tasks`, `timer_tasks`, `job_events`, `cluster_nodes` — all partitioned by `shard_id`. Finding *one* job by `id` is still fast (compute `shard_id`, query one partition). But answering "show me everything interesting right now" requires a script that walks shards, or access to the read model (C.1.3).

This is not a correctness issue; it is an operability regression. Worth calling out because it affects how on-call incidents feel.

### C.2 Soft Losses / Trade-offs

#### C.2.1 Optimistic `version` column on jobs

Current: every job row carries a `version: Long` that increments on every write. Domain code uses it for optimistic concurrency control — "update this job only if `version` matches what I read."

New: the `version` column is gone. Concurrency is enforced at the shard level by the `range_id` fence + the mailbox's single-writer discipline. Inside a shard, the mailbox serializes all writes, so optimistic locking at the row level is redundant.

**Functionally strictly better in the happy path.** The one thing it rules out is a future feature where a client wants to do "update this job only if my view is fresh" at the API level — that primitive no longer exists. If such a feature is ever wanted, it would have to be re-introduced as an application-level check inside the shard handler. Worth noting, not worth worrying about unless a specific use case shows up.

#### C.2.2 DB-enforced checkpoint chain uniqueness

Current: `UNIQUE(job_id, COALESCE(previous_checkpoint_id, 'ROOT'))` is a hard database constraint (§7 of `DESIGN.md`). The database itself refuses to let two checkpoints point to the same predecessor. Defense-in-depth — even a buggy handler cannot corrupt the chain.

New: the mailbox owner is the single writer for its shard, so chain integrity is enforced *in code*, inside the checkpoint-save handler. Functionally equivalent **as long as the mailbox invariant holds** — which it must, because it's load-bearing for the whole architecture.

The loss is a layer of defense-in-depth. A bug in the mailbox loop that allowed concurrent writes would corrupt a chain that the DB constraint would have caught. Mitigation: treat the mailbox's single-writer property as a test-level invariant, with targeted integration tests that try to violate it.

#### C.2.3 Best-case dispatch latency is slightly higher

Current: a worker polling `acquirePendingJobs` sees a newly-created job within one `SELECT ... FOR UPDATE SKIP LOCKED` round-trip — typically tens of milliseconds.

New: job creation → History write → `TransferQueueProcessor` wakes (notification or up to 500ms idle sleep) → `OfferJob` RPC to Matching → parked poller delivery. The happy path is still sub-second, but the *floor* is higher than a direct SQL acquire. Under load with backoff or re-offering, dispatch latency can extend further.

For most workloads this doesn't matter — durable execution is not a latency-critical primitive. For workloads that need tight time-to-dispatch, it's worth measuring on representative traffic before committing.

#### C.2.4 `input_data` travels over inter-pod RPC — **resolved via lazy fetch**

This was originally flagged as a soft loss: in the current design `input_data` is read from the jobs row during acquire and stays inside the one process; the naive port would attach `input_data` to the `OfferJob` RPC, forcing matching to buffer payloads for every pending offer and pushing worst-case matching RAM into the tens of GB under realistic workloads.

**The design resolves this by committing to lazy fetch** (see §5.2.1, §5.3, §7.2, §8.2):

- `OfferJob` carries **metadata only** (`job_id`, `history_shard_id`, `job_type`, `visibility_time`, `attempt`). No payload.
- `PollJob.Response` also carries metadata only — matching's `ReadyQueue` never holds `input_data`.
- When the worker decides to actually run the job, it sends `AcceptJob` to the history shard owner. The history shard's `AcceptJob.Response` returns `input_data` alongside the lease deadline and range token.
- Result: matching's aggregate RAM is bounded to metadata-only (~20 MB per shard worst case), regardless of the job's input size.

**Why not external blob store?** It is still a fine pattern for very large payloads (many MB or more), and clients with such workloads should use it. But for medium payloads (say 1 KB – 1 MB), forcing clients to adopt an external object store just to use LogPlay is a significant usability tax. Lazy fetch lets the server handle payloads of that size directly, transparently, without impacting matching's footprint.

**What lazy fetch costs:** a slightly larger `AcceptJob` response body. There is **no extra RPC round-trip** — `AcceptJob` already exists for the `PENDING → ACQUIRED` transition; we just widen its response. The history shard already has the jobs row in memory (it just wrote the fenced batch against it), so fetching `input_data` is free at that point.

**When to use the external blob store pattern instead:** when `input_data` exceeds ~1 MB in typical use. Above that size, the over-the-wire cost of shipping the payload on every `AcceptJob` (even without RAM pressure) becomes the bottleneck, and the blob-store pattern is preferable. Clients with such workloads should put payloads in S3/GCS and pass a reference URL in `input_data`.

#### C.2.5 Single-process simplicity for small deployments

Current: one fat JAR against H2 is a viable dev/test and small-prod deployment. Startup is `./gradlew :logplay-server-h2:run` and everything runs in one process against one in-JVM database.

New: *can* be deployed on a single pod (all `N_H` shards owned by one pod, `cluster_nodes` has one row) — but the conceptual machinery is all still there. Membership table, shard acquisition via CAS, range_id fencing, mailbox serialization, three background loops per shard. For a user with one tenant and modest scale, this is a significant complexity tax paid whether they need it or not.

The architecture is aimed at horizontal scalability; for single-tenant small-scale use it is overkill, and the current design is simpler and fine. A migration should not force all users to adopt it — keeping the current server as a "small deployments" option is worth considering.

#### C.2.6 Testing surface complexity

Current test matrix: domain unit tests (pure Kotlin, no DB) + integration tests per backend (H2, Postgres) exercising the HTTP surface and gateways.

New test matrix adds: membership convergence under churn, shard acquisition races, split-brain fencing correctness, state reconstruction on shard takeover, `TransferQueueProcessor` backoff behavior, matching queue eviction and backpressure, lease expiry timing, inter-pod RPC failures, clock skew across pods. These are harder to test — many involve wall-clock timing, multi-process scenarios, and concurrency.

Not a functional regression, but a real cost. The integration test harness for the new design is meaningfully more complex than `AbstractIntegrationTest.kt`'s 70-ish cases today.

### C.3 Moved But Equivalent

These items are worth naming only because someone reading C.1 and C.2 might wonder if they're losses too. They are not — they simply re-implement against new primitives.

| Current design | New design | Notes |
|---|---|---|
| Two-phase dead worker cleanup (condemn → delete) | Per-shard `LeaseExpiryReaper` + cluster `DeadWorkerSweeper` | Same race-condition safety, different primitive. Lease expiry is simpler than condemnation; no `condemned` flag needed. |
| Checkpoint linked chain (domain invariant) | Preserved; enforced by mailbox single-writer | See C.2.2 for the defense-in-depth caveat. |
| `retries` reset on checkpoint save | Preserved; domain rule, moves into checkpoint handler | Unchanged semantics. |
| Event atomicity with state transition | Preserved via fenced batch writes (§6.3/§6.4) | Same guarantee, stronger mechanism (atomic batch across `jobs` + `job_events` rows). |
| Event chronological ordering | Preserved; *sharper* via `event_seq` | Today: ordering is by `created_at`, subject to clock granularity. New: `event_seq` is a monotonic sequence per job within its shard — strict total order, no clock dependency. |
| Deterministic `job_id` from `idempotency_key` (UUIDv5) | Identical | Same generator, same namespace, same property. |
| Abort doesn't notify the running worker | Identical | Worker discovers on its next heartbeat/checkpoint, same as today. |
| Worker-voluntary release | Identical | Maps to `ReleaseJob` RPC routed to the history shard. |

### C.4 Decisions Required Before v1

Of the items in C.1 and C.2, these are the ones where this document should make an explicit commitment — not drift. A v1 that silently inherits the current design's users will hit these cold.

**D1 — Multi-tenancy model.** **Decision: Option 3 — `groupId` is re-introduced as a first-class concept.** It is included in job ID derivation (`job_id = UUIDv5(groupId, idempotencyKey)`), matching shard assignment (`hash(groupId, type) mod N_M`), and `PollJob` requests. Tenant isolation is enforced at the server level: a worker registered for group A cannot receive group B's jobs. This resolves C.1.1 completely.

**D2 — Idempotency key scope.** **Resolved by D1.** Since `groupId` is part of the job ID derivation, idempotency keys are scoped per group — the same key in different groups produces different job IDs. No collision risk. This matches the current design exactly and resolves C.1.2.

**D3 — Ad-hoc query surface.** C.1.3 is the regression users will notice first. Three options:

1. **Ship CQRS read model as part of v1.** Every history shard publishes `job_events` to a downstream store (Elasticsearch, ClickHouse, Postgres replica). A query API on the Frontend serves admin queries from the read model. Cost: a second storage system becomes required infrastructure.
2. **Fan-out query API on the Frontend.** For a handful of common patterns (`GET /jobs?status=PENDING`, `GET /jobs?name~=...`), the Frontend fans out to every history shard, aggregates, returns results. Slow but no extra infrastructure. Cost: query latency grows with `N_H`; bad for interactive UI, acceptable for periodic scripts.
3. **Explicitly drop the query surface in v1.** Document the regression; tell users to instrument their own read model via the existing `job_events` RPC. Cost: migration blocker for any team that currently uses `SELECT * FROM jobs` in production.

**Recommendation:** Option 2 (fan-out API) as the v1 surface — good enough for monitoring and ad-hoc debugging — with Option 1 (read model) as the documented path for interactive admin UIs, built as needed.

**D4 — Large-payload handling — committed.** The design commits to **lazy fetch** as the mechanism for shipping `input_data` (see C.2.4). Summary of the commitment:

- `OfferJob` and `PollJob.Response` carry **metadata only**. Matching's `ReadyQueue` never holds `input_data`. This caps matching's worst-case RAM at ~20 MB per shard (§5.3 *In-memory footprint and retention*).
- `AcceptJob.Response` carries `input_data` alongside the lease deadline. The payload flows once per dispatch, from the history shard owner directly to the worker, with no extra RPC round-trip (the `AcceptJob` request already exists; the response just grows).
- **Soft guidance for clients**: keep `input_data` under ~1 MB in typical use. Above that, use an external blob store (S3, GCS, etc.) and pass a reference URL in `input_data`. The server does not enforce a hard limit, but workloads routinely pushing megabytes through every `AcceptJob` will feel it in latency even though matching RAM stays bounded.

**No 256 KB soft limit.** An earlier draft of this section recommended 256 KB as a protective cap. That was a band-aid against a RAM problem that lazy fetch eliminates — the fix belongs in the architecture, not in an artificial size restriction on the API.

**D5 — Coexistence strategy with the current server.** Given C.2.5 (single-process simplicity) and the migration cost implied by C.1.3 and C.1.1, this doc should take a position on whether the current server is retired or kept as a small-deployment option. If kept, the SDK must present both server flavors behind one client interface. If retired, the migration tooling must handle tenants who depended on the ad-hoc query surface.

**Recommendation:** Keep the current server (`logplay-server-h2`, `logplay-server-postgres`) as supported small-deployment flavors. The stateful-server architecture ships alongside them as the scaling option, not the replacement. Deprecate only when a migration path exists for the items in C.1.
